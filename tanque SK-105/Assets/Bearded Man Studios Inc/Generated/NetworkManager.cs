...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] TankNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is TankNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (TankNetworkObject.Length > 0 && TankNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(TankNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<TankBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateTank instead, its shorter and easier to type out ;)")]
		public TankBehavior InstantiateTankNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(TankNetworkObject[index]);
			var netBehavior = go.GetComponent<TankBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<TankBehavior>().networkObject = (TankNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of Tank
		/// </summary>
		/// <returns>
		/// A local instance of TankBehavior
		/// </returns>
		/// <param name="index">The index of the Tank prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public TankBehavior InstantiateTank(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (TankNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: Tank. Add them at the NetworkManager prefab.");
				return null;
			}
